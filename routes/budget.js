...
// ──────────────────────────────────────────────
// GET /api/budget/chantier/:chantierId/budget
// ──────────────────────────────────────────────
router.get('/chantier/:chantierId/budget', checkResourceAccess, async (req, res) => {
  try {
    const { chantierId } = req.params;

    const [depensesResult, revenusResult, budgetResult] = await Promise.all([
      pool.query(`
      ...
      // ✅ FIX : COALESCE sur budget_total ET budget (les deux peuvent exister)
      pool.query(`
        SELECT COALESCE(budget_total, budget, 0) as budget
        FROM chantiers
        WHERE id = $1
      `, [chantierId]),
    ]);

    const budget_alloue    = parseFloat(budgetResult.rows[0]?.budget || 0);
    const total_depenses   = depensesResult.rows.reduce((sum, r) => sum + parseFloat(r.total_depenses || 0), 0);
    const total_revenus    = revenusResult.rows.reduce((sum, r) => sum + parseFloat(r.total_revenus  || 0), 0);
    const solde            = total_revenus - total_depenses;
    const pourcentage_utilise = budget_alloue > 0
      ? ((total_depenses / budget_alloue) * 100).toFixed(2)
      : 0;
...
// ──────────────────────────────────────────────
// GET /api/budget/etape/:etapeId/budget
// ──────────────────────────────────────────────
router.get('/etape/:etapeId/budget', async (req, res) => {
  try {
    ...
    res.json({
      ...b,
      solde: parseFloat(b.total_revenus) - parseFloat(b.total_depenses),
    });
  } catch (error) {
...
router.post('/depenses', requireAdminOrChef, async (req, res) => {
  try {
    ...
    if (!chantier_id || !montant || !categorie)
      return res.status(400).json({ error: 'Champs requis manquants (chantier_id, montant, categorie)' });
...
router.post('/revenus', requireAdminOrChef, async (req, res) => {
  try {
    ...
    if (!chantier_id || !montant || !source)
      return res.status(400).json({ error: 'Champs requis manquants (chantier_id, montant, source)' });
...
// ──────────────────────────────────────────────
// GET /api/budget/statistiques/budget
// ──────────────────────────────────────────────
router.get('/statistiques/budget', requireAdminOrChef, async (req, res) => {
  try {
    const [depensesCat, revenusSrc, topDepenses, topRevenus, tendance] = await Promise.all([
      pool.query(`SELECT categorie, SUM(montant) as total, COUNT(*) as nb FROM depenses GROUP BY categorie ORDER BY total DESC`),
      pool.query(`SELECT source, SUM(montant) as total, COUNT(*) as nb FROM revenus GROUP BY source ORDER BY total DESC`),
      pool.query(`SELECT d.*, c.nom as chantier FROM depenses d LEFT JOIN chantiers c ON d.chantier_id = c.id ORDER BY d.montant DESC LIMIT 5`),
      pool.query(`SELECT r.*, c.nom as chantier FROM revenus  r LEFT JOIN chantiers c ON r.chantier_id = c.id ORDER BY r.montant DESC LIMIT 5`),
      pool.query(`
        ...
        FROM (
          SELECT date, montant, 'depense' as type FROM depenses
          UNION ALL
          SELECT date, montant, 'revenu'  as type FROM revenus
        ) combined
    ...
    res.json({
      ...
      total_depenses: depensesCat.rows.reduce((s, r) => s + parseFloat(r.total || 0), 0),
      total_revenus:  revenusSrc.rows.reduce((s, r) => s + parseFloat(r.total || 0), 0),
    });
  } catch (error) {
